...
> file/linear addresses as shown by radare2 (`pdf @ <addr>`). Companion: `_RE_OPCODES_WORLD.md`
> (Spinner = the same 14-bit angle model + turn animation), `_RE_INDEX.md`, `_RE_NOTES.md`.
...
shorter arc (same loop as the Spinner, see `_RE_OPCODES_WORLD.md`).
