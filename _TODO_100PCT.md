...
| B4 | **UseItem world-verb.** | `SelectionHandler2D/3D`, `FlatMap`/`DungeonMap` | ✅ **DONE.** Both selection handlers now offer the `MapPopup_UseItem` verb when the player holds an item AND the zone has the `UseItem` (0x10) trigger bit. `FlatMap.TileTriggered`/`DungeonMap.TileTriggered` carry the held item id into the UseItem `EventSource.AssetId` so the zone chain's `query used_item == X` (Querier.QueryUsedItem) matches. Unblocks tool-on-obstacle puzzles (pick-axe/screwdriver/staff — Toronto endgame, Gratogel library). Build clean, 305 tests, smoke 13/13. Live puzzle exercise awaits a relevant scene (Phase-3 run). |
