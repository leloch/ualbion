...
// then mastery multiplier + mastery growth as already documented in _RE_COMBAT.md.
```
...
per `_RE_ASK_SURRENDER.md`. The surrender win-condition therefore **can** fire, and only in the
...
`_RE_ASK_SURRENDER.md` reads the runtime behaviour key from combatant `[+0x12]` and notes the
...
- Wire it into post-strike combat AI exactly as `_RE_ASK_SURRENDER.md` recommends: when an attacking
...
records). The link from strategy-9 → surrender check is CONFIRMED in `_RE_ASK_SURRENDER.md`.
COULD NOT DETERMINE here: the exact ending map/event id chained after surrender (it is supplied at
runtime by the boss encounter's event-chain args, not a constant — see `_RE_ASK_SURRENDER.md`).
