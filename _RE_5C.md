...
> All radare2 against `albion_aaa`. Companion to _RE_COMBAT.md and _RE_5B.md.
...
matches _RE_COMBAT), **21 Light = `0x64359`**.
...
whose adder `fcn.000607da` drops the call when the entry is non-empty — see _RE_5B §5b).
...
34=0xa068b (Teleporter, matches _RE_5B), 35=0x64621, 36=0xa1021 (QuickWithdrawal),
**37 Levitation = NULL**, **38 Unused38 = NULL**, 39=0xa1134 (GoddessWrath, matches _RE_5B),
...
targets=0x08} — both consistent with the _RE_5B target-mask decode.)
...
- A matching key is only consumed if it carries ITEMLIST flag 0x10 ("vanish when used up" —
  same bit the spell-item charge code uses, _RE_5B §3).
