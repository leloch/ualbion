...
| ~~Unknown1C HomeNpcIndex wiring~~ | `Party.cs` | DONE (leave side) — RE 5D §6. **Join** is already script-driven: recruit chains run `modify_npc_off Set <slot> <map>` to hide the home NPC (verified in the decompiled Edjirr chain: `add_party_member Sira` → `modify_npc_off Set 8 Edjirr`). **Leave** now re-enables it: `RemovePartyMemberEvent.Unk6` carries the flat home index `mapId*96+slot` (Sira=26888=280*96+8), which `Party.OnRemovePartyMember` decodes via the new pure `HomeNpcIndex.TryDecode` and clears with `ModifyNpcOffEvent(Clear)`. No context-slot derivation, no save-compat risk (index comes from the script data). `HomeNpcIndexTests` (+7). The `CharacterSheet.Unknown1C` field itself stays as loaded — behaviour is driven by the data-carried index, not the field, so save bytes are untouched. |
