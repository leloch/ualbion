...
> All addresses are file/linear addresses as shown by radare2 (`pd @ <addr>`). Companion: docs/re/RE_ASK_SURRENDER.md (dispatcher + table), docs/re/RE_COMBAT.md (RNG/PercentRoll/RandomVary).

## KNOWN FACTS (running list)

### Dispatch & record layout (CONFIRMED)
- Master map-event dispatcher = `fcn.000312c2`; handler-pointer table at `0x13da7c` indexed by event-type byte (from docs/re/RE_ASK_SURRENDER.md).
...
- `fcn.00039941(memberId)` = "is party slot occupied" (returns member-handle/nonzero); from docs/re/RE_ASK_SURRENDER.md.
