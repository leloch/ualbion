...
Status: **COMPLETE (2026-07-05)** — contradiction settled in docs/re/RE_NOTES.md's favour with one
...
## Known facts going in (from docs/re/RE_NOTES.md, already CONFIRMED there)
...
- Spell SFX: real samples 38, 201..268 via per-spell handlers (table in docs/re/RE_NOTES.md).
...
105 bytes. All the strike-pipeline "PlaySound" calls in docs/re/RE_COMBAT.md are calls to THIS.
...
text-only. docs/re/RE_NOTES.md was right; docs/re/RE_COMBAT.md's "PlaySound" naming was wrong.

### F3. fcn.0004dec9 (ApplyDamage): Sample 268 plays on EVERY damaging hit, not just death — CORRECTION to docs/re/RE_NOTES
...
death scream and no extra sound at LP==0. docs/re/RE_NOTES.md "Combatant dies → 268" is a
...
| Spell cast | SAMPLES | per-spell table | unchanged (docs/re/RE_NOTES.md table stands) |
...
8. **Doc drift**: docs/re/RE_COMBAT.md's "plays sound NNN" wording (lines ~314/320/705/728/732/1239,
   §"strike pipeline") should be re-worded to ShowCombatMessage/SYSTEXTS; docs/re/RE_NOTES.md's
