...
> radare2 against `albion_aaa` (MAIN.EXE, DOS LE). Conventions per docs/re/RE_5A.md / docs/re/RE_5D.md.
...
| tile = 64 world units (per docs/re/RE_5A) | so 1000/frame ≈ 15.6 tiles/frame (orb is fast), 250/frame ≈ 3.9 tiles/frame | |
...
- MapNpc V2 byte1 → **NpcState+0x07 = Sound = the sound-set row 0..12** (per docs/re/RE_5D §8).
