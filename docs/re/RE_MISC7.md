# docs/re/RE_MISC7.md — misc small unknowns batch (queries, actions, map opcodes, torch burn, camp scene)
...
- Map-event dispatcher fcn.000312c2; handler table **0x13da7c** (entry N = opcode N handler).
  From docs/re/RE_ASK_SURRENDER.md: 0x10 CloneAutomap = 0x3b05b, 0x17 Wipe = 0x3b984, 0x1A Pause = 0x3bbb4.
- Rest executor 0x68b05 (fade-out fcn.0009251d, per-member recovery fcn.00037958, then
  AdvanceClock fcn.000439b3). Hour tick = fcn.00043acc. (docs/re/RE_5C.md §4)
- Party light-item total = fcn.00038f24 (the item-light accumulator, docs/re/RE_5C.md item 1.3).
...
### 0x26 @ 0x3d796 = **Compare(LeaderSkill[imm], op, arg)** (CONFIRMED, was in docs/re/RE_5C)
...
### 0x27 @ 0x3d7ce = **Compare(LeaderAttribute[imm], op, arg)** (CONFIRMED — upgraded from docs/re/RE_5C's INFERRED)
...
4. Per-member rest recovery loop (fcn.00037958) — heals BEFORE the clock advance (as in docs/re/RE_5C §4).
...
- docs/re/RE_5C.md §3` said 0x20/0x25 "fallthrough no-op" — they are no-ops returning **TRUE**
