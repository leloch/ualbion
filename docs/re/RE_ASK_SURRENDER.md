...
- Behaviour strategy id 9 (runtime combatant `[+0x12]==9`, on-disk MONCHAR field, indexed 8) is the final boss. The mapping disk-MONCHAR `+0x0C` → runtime combatant `+0x12` is asserted in docs/re/RE_INDEX.md; I confirmed the runtime read at `+0x12` but did not trace the setup copy.
