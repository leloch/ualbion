...
> address-stamped. This doc SUPERSEDES the conflicting readings in docs/re/RE_3D_COLLISION.md ("bit dir+11,
> dir=direction") and docs/re/RE_COLLISION_DATA.md ("mask 0x78 union") and the docs/re/RE_5D.md §4 sketch.
...
### Why this reconciles ALL the dumped data (from docs/re/RE_COLLISION_DATA.md §2)
...
| **Plates/jewels/mud floors `Unk1=0x10/0xF0`** | 0x10/0xF0 | **PASSES** | blocks | resolves the docs/re/RE_COLLISION_DATA §5 caveat exactly — `&0x78` wrongly blocks these |
...
// (fcn.0001e28a keyboard / fcn.0001db96 mouse: v = const · S/D, dx16 = v·cos etc., see docs/re/RE_SMOOTH_MOVE.md)
...
- Gate: same `(0x08 << class)` rule on the object's raw Collision byte. Party class 0: objects with
  ...
  pass. (Matches the 1887-record dump in docs/re/RE_COLLISION_DATA.md.)
