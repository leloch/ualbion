...
> Companion to docs/re/RE_ASK_SURRENDER.md (dispatcher table) and docs/re/RE_INDEX.md.

## KNOWN FACTS (running list)

### Dispatch / event-record model (CONFIRMED)
- Map-event dispatcher = **`fcn.000312c2`**. Handler table at **`0x13da7c`**, indexed by event-type byte (`call dword [type*4 + 0x13da7c]`, evidence `0x3139e..0x313a1`). Table re-dumped from binary — matches docs/re/RE_ASK_SURRENDER.md.
