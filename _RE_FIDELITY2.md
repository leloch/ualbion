# _RE_FIDELITY2.md — Trapped chests + the "SLP" pool
...
- **Task 1:** the trap mechanic is real and fully RE'd (it already was, in `_RE_5C.md §2`).
...
| +0x11 (u8)  | **Action points this turn** (NOT training points) | ApplyLevelUp stores `byte[edx+0x11]=clamp(level/w[+0xE2],1,4)`; also `_RE_COMBAT.md:858` |
...
| +0xE6 (u16) | SpellPointsPerLevel | (per `_RE_COMBAT.md`, unchanged) |
...
**not a hardcoded constant**. (Consistent with `_RE_NOTES.md` TrapEvent type 5.)
...
- Trap trigger conditions, evasion (DEX roll), one-shot disarm, result codes, retry semantics,
  Lockpick-bypasses-trap: **CONFIRMED** (door.c `0x5ab33`, re-verified; matches `_RE_5C.md §2`).
...
`_RE_COMBAT.md:2245` as "LearnCloseCombat"). It is the **skill-trainer** NPC service:
```
